...
// Bun loads .env automatiquement (pas besoin de dotenv)
...
const importPattern = `${import.meta.dirname}/{events,commands}/**/*.{ts,js}`;
...
async function loadFiles(path: string): Promise<void> {
  ...
  await Promise.all(
    ...
    files.map((file) => {
      ...
      // ESM : pas de require.cache, cache-busting via query string
      import(`${newFileName}?t=${Date.now()}`);
    }),
  );
}
